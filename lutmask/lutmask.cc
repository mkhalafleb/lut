...
LutMask::LutMask(uint16_t mask, unsigned int size)
    : _lutmask(mask), domainc_(size, lutdomain::LutDomain::DefaultDomain) {

  std::string o_range("");

  if (size > lutdomain::LutDomain::MaxLutSize) {
    // Throw Exception, class can't be constructed
    o_range += "Illegal Mask Size: " + std::to_string(size) + " , Maximum is " +
               std::to_string(lutdomain::LutDomain::MaxLutSize);
    throw std::out_of_range(o_range);
  }
  if (mask > lutmask::LutMask::lut_size_mask[size]) {
    std::ostringstream ss;
    ss << std::hex << mask;
    o_range += "Illegal Mask: " + ss.str() +
               " with mask size: " + std::to_string(size);
    throw std::out_of_range(o_range);
  }
  // Check if the mask fits within size.
}

LutMask::LutMask(uint16_t mask, const lutdomain::LutDomain &domain)
    : _lutmask(mask), domainc_(domain){};

bool LutMask::IsGnd() const { return (_lutmask == 0); }
...
void LutMask::ValidatePosToSize(unsigned int pos) const {
  if (pos >= domainc_.GetSize()) {
    std::string error("");
    error += "Can't Create independent mask at pos " + std::to_string(pos) +
             " With size " + std::to_string(domainc_.GetSize());
...
lutmask::LutMask LutMask::CreateMaskIndependentOfPos(unsigned int pos) const {
  ...
  uint16_t work_mask =
      LutMask::ShrinkMaskAtPos(LutMask::position_mask[pos], pos);
...
bool LutMask::IsSingleLiteral() const { return (domainc_.GetSize() == 1); }
...
lutmask::LutMask LutMask::InvertMask() const {
  return (lutmask::LutMask(
      (~_lutmask) & LutMask::lut_size_mask[domainc_.GetSize()], domainc_));
}
