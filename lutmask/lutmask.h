...
public:
  ...
  static constexpr uint16_t lut_size_mask[lutdomain::LutDomain::MaxLutSize+1] = {0x0001, 0x0003, 0x000F, 0x00FF, 0xFFFF};
  
  
  // Mask is on a Look Up Table for values of
  // 0xFF00 is when D =1 and D = 0
  // 0XF0F0 is when C = 1 and C = 0 and so on
  static constexpr uint16_t position_mask[lutdomain::LutDomain::MaxLutSize] = {0xAAAA, 0xCCCC, 0xF0F0, 0xFF00};
